...
w_status_t system_init(void) {
  ...
  status |=
      init_with_retry_param((w_status_t(*)(void *))can_handler_init, &hfdcan1);
  ...
  task_status &=
      xTaskCreate(flight_phase_task, "flight phase", 512, NULL,
                  flight_phase_task_priority, &flight_phase_task_handle);

  task_status &=
      xTaskCreate(imu_handler_task, "imu handler", 512, NULL,
                  imu_handler_task_priority, &imu_handler_task_handle);

  task_status &= xTaskCreate(can_handler_task_rx, "can handler rx", 512, NULL,
                             can_handler_rx_priority, &can_handler_handle_rx);

  task_status &= xTaskCreate(can_handler_task_tx, "can handler tx", 512, NULL,
                             can_handler_tx_priority, &can_handler_handle_tx);

  task_status &= xTaskCreate(movella_task, "movella", 2560, NULL,
                             movella_task_priority, &movella_task_handle);

  task_status &= xTaskCreate(log_task, "logger", 2048, NULL, log_task_priority,
                             &log_task_handle);
