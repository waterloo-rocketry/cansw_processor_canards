...
void can_handler_task_rx(void *argument) {
    ...
    bool temp = can_init_stm(&hfdcan1, can_handle_rx_isr);
    for (;;) {
        can_msg_t *rx_msg;
...
void can_handler_task_tx(void *argument) {
    ...
    for (;;) {
        ...
        if (xQueueReceive(busQueue_tx, &tx_msg, 1000) ==
            pdTRUE) { // Returns pdTRUE if we got a message, pdFALSE if timed out
