...
void model_dynamics_jacobian(
    ...
    write_pData(
        pData, 0, 0, SIZE_QUAT, SIZE_QUAT, &q_q.flat[0]
    ); // J_x(1:4,1:4) = q_q; % column q (attitude)
    write_pData(
        pData, 0, 4, SIZE_QUAT, SIZE_VECTOR_3D, &q_w.flat[0]
    ...
    write_pData(
        pData, 4, 12, SIZE_VECTOR_3D, SIZE_1D, &w_delta.array[0]
    ...
    write_pData(pData, 10, 0, SIZE_1D, SIZE_QUAT, &alt_q[0]); // J_x(11,1:4) = alt_q; % column q
    write_pData(
        pData, 10, 7, SIZE_1D, SIZE_VECTOR_3D, &alt_v.array[0]
    ); // J_x(11,8:10) = alt_v; % column v
    write_pData(pData, 10, 10, SIZE_1D, SIZE_1D, &alt_alt); // J_x(11, 11) = alt_alt; % column alt
    ...
    write_pData(pData, 11, 11, SIZE_1D, SIZE_1D, &Cl_Cl); // J_x(12,12) = Cl_cl; % column Cl
    ...
    // write to pData: a scalar
    write_pData(
        pData, 12, 12, SIZE_1D, SIZE_1D, &delta_delta
