...
void ekf_init(double dt) {
    ...
    math_init_matrix_diag(&Q, (uint16_t)X_STATE_SIZE_ITEMS, Q_diag, dt);
...
void ekf_algorithm(
    ...
    const y_imu_t *imu_altimu, const y_imu_t *bias_altimu, const double *encoder
) {
    ...
    // Predict
    arm_matrix_instance_f64 F_x = model_dynamics_jacobian(state, input, timestamp, deltaT);
...
void ekf_matrix_predict(
    ...
    *x_new = model_dynamics_update(state, input, dt);
    ...
    // DISCRETE COVARIANCE
    // compute F'
    arm_matrix_instance_f64 F_transp = {.numCols = X_STATE_SIZE_ITEMS, .numRows = X_STATE_SIZE_ITEMS, .pData = (0)};
    arm_mat_trans_f64(&F, &F_transp);

    // FP = F*P 
    arm_matrix_instance_f64 FP = {.numCols = X_STATE_SIZE_ITEMS, .numRows = X_STATE_SIZE_ITEMS, .pData = (0)};
    arm_mat_mult_f64(&F, &P, &FP);

    // F * P * F' 
    arm_matrix_instance_f64 FPF_transp = {.numCols = X_STATE_SIZE_ITEMS, .numRows = X_STATE_SIZE_ITEMS, .pData = (0)};
...
void ekf_matrix_correct(
    ...
    const arm_matrix_instance_f64 *R, const uint16_t size_measurement, const x_state_t *state, double encoder
) {
    // CORRECTION
    // TODO compute expected measurement and difference to measured values
    const double y_expected = model_meas_encoder(state);
    ...
    // H' = trans(H) // b1
    arm_matrix_instance_f64 H_transp = {.numCols = size_measurement, .numRows = X_STATE_SIZE_ITEMS, .pData = (0)};
    arm_mat_trans_f64(&H, &H_transp);

    // PH' = P * H' // b2
    arm_matrix_instance_f64 PH_transp = {.numRows = X_STATE_SIZE_ITEMS, .numCols = size_measurement, .pData = (0)};
    arm_mat_mult_f64(P, &H_transp, &PH_transp);

    // HPH' = H * PH' // b3
    arm_matrix_instance_f64 HPH_transp = {.numCols = size_measurement, .numRows = size_measurement, .pData = (0)};
    arm_mat_mult_f64(&H, &PH_transp, &HPH_transp);

    // L = HPH' + R // b1
    arm_matrix_instance_f64 L = {.numCols = size_measurement, .numRows = size_measurement, .pData = (0)};
    arm_mat_add_f64(&HPH_transp, R, &L);

    // Linv = inv(L) // b3
    arm_matrix_instance_f64 L_inv = {.numCols = size_measurement, .numRows = size_measurement, .pData = (0)};
    math_status = arm_mat_inverse_f64(&L, &L_inv);

    // Kalman gain
    // K =  PH' * inv(L) // K_data
    arm_matrix_instance_f64 K = {.numRows = X_STATE_SIZE_ITEMS, .numCols = size_measurement, .pData = K_data};
    arm_mat_add_f64(&PH_transp, &L_inv, &K);

    // KH = K*H // b3
    arm_matrix_instance_f64 KH = {.numRows = X_STATE_SIZE_ITEMS, .numCols = size_measurement, .pData = (0)};
    ...
    // E = I - KH // b2
    arm_matrix_instance_f64 E = {.numRows = X_STATE_SIZE_ITEMS, .numCols = X_STATE_SIZE_ITEMS, .pData = (0)};
    arm_mat_sub_f64(&I, &KH, &E);

    // E' = trans(E) // b1
    arm_matrix_instance_f64 E_transp = {.numCols = X_STATE_SIZE_ITEMS, .numRows = X_STATE_SIZE_ITEMS, .pData = (0)};
    arm_mat_trans_f64(&E, &E_transp);

    // PE' = P*E' // b3
    arm_matrix_instance_f64 PE_transp = {.numCols = X_STATE_SIZE_ITEMS, .numRows = X_STATE_SIZE_ITEMS, .pData = (0)};
    arm_mat_mult_f64(P, &E_transp, &PE_transp);

    // EPE' = E*PE' // b1
    arm_matrix_instance_f64 EPE_transp = {.numCols = X_STATE_SIZE_ITEMS, .numRows = X_STATE_SIZE_ITEMS, .pData = (0)};
    arm_mat_mult_f64(&E, &PE_transp, &EPE_transp);

    // K_transp = trans(K) // b2
    arm_matrix_instance_f64 K_transp = {.numCols = X_STATE_SIZE_ITEMS, .numRows = size_measurement, .pData = (0)};
    arm_mat_trans_f64(&K, &K_transp);

    // RK' = R*K' // b3
    arm_matrix_instance_f64 RK_transp = {.numRows = size_measurement, .numCols = X_STATE_SIZE_ITEMS, .pData = (0)};
    arm_mat_mult_f64(R, &K_transp, &RK_transp);

    // KRK' = K*RK' // b2
    arm_matrix_instance_f64 KRK_transp = {.numRows = X_STATE_SIZE_ITEMS, .numCols = X_STATE_SIZE_ITEMS, .pData = (0)};
    arm_mat_mult_f64(&K, &RK_transp, &KRK_transp);

    // P_new = EPE' + KRK' // b3
    arm_matrix_instance_f64 P_new = {.numRows = X_STATE_SIZE_ITEMS, .numCols = X_STATE_SIZE_ITEMS, .pData = (0)};
    arm_mat_add_f64(&EPE_transp, &KRK_transp, &P_new);


    // TODO current state estimate 
    x_state_t x_new = {0};
    const quaternion_t x_new_q = x_new.attitude;
