...
static w_status_t get_adc_current(uint32_t *adc_current_mA) {
    ...
    uint32_t voltage_mV = ((float)(adc_value) / ADC_MAX_COUNTS) * ADC_VREF * 1000.0f; // TODO this loses precision for no good reason ??
...
void watchdog_kick(void) {
  ...
  if(timer_get_ms(&current_time) != W_SUCCESS){
      //logging errors how? panth figure it out
      return;
  }
...
void watchdog_register_task(TaskHandle_t task_handle, uint32_t timeout_ticks){
  ...
  for (uint8_t i = 0; i < num_watchdog_tasks; i++) {
    if (watchdog_tasks[i].task_handle == task_handle) {
      // Update the timeout if the task is already registered
  ...
  float current_time = 0.0f;
    ...
    watchdog_tasks[num_watchdog_tasks].is_kicked = true;  // Give it initial kick
...
static w_status_t check_watchdog_tasks(void) {
  w_status_t status = W_SUCCESS;
  float current_time = 0.0f;
  ...
  for(uint8_t i = 0; i < num_watchdog_tasks; i++){
      if(watchdog_tasks[i].task_handle != NULL){
        ...
        if(!watchdog_tasks[i].is_kicked && 
           (ticks_elapsed > watchdog_tasks[i].timeout_ticks)){
            //logging errors how? panth figure it out
           
            //sending a message over CAN
            can_msg_t msg = {0};
            if (false == build_general_board_status_msg(
                             PRIO_HIGH, (uint16_t)current_time, 
                             E_WATCHDOG_TIMEOUT, i, &msg)) { //figure this out what goes here 
                return W_FAILURE;
            }
