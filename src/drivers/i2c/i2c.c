 ...
 * @param handle Bus handle pointer
...
/**
 * @brief Wait for transfer completion with timeout handling
 * @param handle Pointer to bus handle structure
 * @param bus Bus index for error tracking
 * @return Transfer status
 */
...
w_status_t i2c_init(i2c_bus_t bus, I2C_HandleTypeDef *hal_handle, uint32_t timeout_ms)
{
    ...
    HAL_I2C_RegisterCallback(hal_handle, HAL_I2C_MASTER_TX_COMPLETE_CB_ID,
                             i2c_transfer_complete_callback);
    HAL_I2C_RegisterCallback(hal_handle, HAL_I2C_MASTER_RX_COMPLETE_CB_ID,
                             i2c_transfer_complete_callback);
    HAL_I2C_RegisterCallback(hal_handle, HAL_I2C_ERROR_CB_ID,
                             i2c_transfer_complete_callback);
...
w_status_t i2c_read_reg(i2c_bus_t bus, uint8_t device_addr, uint8_t reg,
...
w_status_t i2c_write_reg(i2c_bus_t bus, uint8_t device_addr, uint8_t reg,
