...
bool compare_quaternion(quaternion_t q1, quaternion_t q2) {
    for (int i = 0; i < 4; i++) {
        if (abs(q1.array[i] - q2.array[i]) > 0.00001) { //tolerance to ensure that each entry matches
            return false;
        }
    }
    return true;
...
TEST(quaternion_normalize_test, HandlesExpectedInput){
    ...
    //ASSERT:
    EXPECT_FLOAT_EQ(quaternion_norm(&actualResult), 1.0); //norm of a normalized quaternion should be 1
